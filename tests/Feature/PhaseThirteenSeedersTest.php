...
test('database seeder creates an idempotent admin user and complete demo commerce dataset', function () {
    ...
    expect(Role::count())->toBe(6)
        ...
        ->and(NavigationMenuItem::count())->toBe(4)
        ->and(HeroSlide::count())->toBe(1)
        ->and(HomepageSection::count())->toBe(1)
        ->and(PromotionalBanner::count())->toBe(1)
        ->and(StaticPage::count())->toBe(6)
        ->and(Faq::count())->toBe(3)
        ->and(ServiceBenefit::count())->toBe(3)
        ->and(Testimonial::count())->toBe(2)
        ->and(StoreLocation::count())->toBe(1)
        ->and(SocialLink::count())->toBe(2)
        ->and(FooterSection::count())->toBe(1)
        ->and(FooterLink::count())->toBe(3)
